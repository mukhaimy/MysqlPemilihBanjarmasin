...
{
    public class ExcelLuarYusna2
    {
        
        public int RunYusna(string filepath, string namafolder, string namafile)
        {
            int total = 0;
            try
            {
                ...
                using (ExcelPackage package = new ExcelPackage(existingFile))
                {
                    ...
                    for (int i = 0; i < totalSheet; i++)
                    {
                        ...
                        int sheetId = i;
                        List<Luar> lstLuar =
                            ReadDptYusna(worksheet, namafolder, namafile, sheetId, worksheet.Name);
        ...
        private List<Luar> ReadDptYusna(ExcelWorksheet worksheet,
            ...
            int sheetId, string sheetName)
        {
            ...
            string kKelurahan = worksheet.Cells[1, 1].Text.Replace("KELURAHAN", "").Trim().ToUpper();
            ...
            // for (int row = 12; row < 16; row++)
            {
                ...
                else if (cell1.Contains("TPS"))
                {

                    kTps = cell1.Replace("TPS", "").Trim();
                ...
                if (worksheet.Cells[row, 1].Text.Contains("NO") || worksheet.Cells[row, 2].Text.Contains("NAM"))
                {
                    continue;
                }
