...
{
    public class ExcelLuar
    {
        #region Syahreza
        //public int RunSyahreza(string filepath, string namafolder, string namafile)
        //{
        //    int total = 0;
        //    try
        //    {
        ...
        //                int sheetId = i;
        //                List<Luar> lstLuar = 
        //                    ReadDptSyahreza(worksheet, namafolder, namafile, sheetId, worksheet.Name);
        //                context.LuarSet.AddRange(lstLuar);
        //                context.SaveChanges();
        //                Console.WriteLine($"ReadSyahreza on: {sheetId}.{worksheet.Name}");
        //                total += lstLuar.Count;
        //            }                    
        //        }
        //        Console.WriteLine();
        ...
        //private List<Luar> ReadDptSyahreza(ExcelWorksheet worksheet,
        ...
        //            p1.no_urut = xno_urut;
        //        }
        //        p1.tps = worksheet.Cells[row, 2].Text;
        //        p1.nama = worksheet.Cells[row, 3].Text;
        //        p1.jenis_kelamin = worksheet.Cells[row, 4].Text;
        //        p1.kelurahan = worksheet.Cells[row, 5].Text;
        //        p1.rt = worksheet.Cells[row, 6].Text;
        //        p1.rw = worksheet.Cells[row, 7].Text;
        //        p1.nik = worksheet.Cells[row, 8].Text;
        //        p1.is_dukung = true;
        ...
        #region HBudi1
        public int RunHBudi(string filepath, string namafolder, string namafile)
        {
            int total = 0;
            try
            {
                ...
                using (ExcelPackage package = new ExcelPackage(existingFile))
                {
                    ...
                    for (int i = 0; i < totalSheet; i++)
                    {
                        ...
                        int sheetId = i;
                        List<Luar> lstLuar =
                            ReadDptHBudi(worksheet, namafolder, namafile, sheetId, worksheet.Name);
                        context.LuarSet.AddRange(lstLuar);
                        context.SaveChanges();
                        Console.WriteLine($"ReadDptHBudi on: {sheetId}.{worksheet.Name}");
                        total += lstLuar.Count;
                    }
                }
                Console.WriteLine();
        ...
        private List<Luar> ReadDptHBudi(ExcelWorksheet worksheet,
            string namafolder, string namafile,
            int sheetId, string sheetName)
        {
            DateTime time0 = DateTime.Now;
            ...
            // for (int row = 12; row < 16; row++)
            {
                ...
                kTps= worksheet.Cells[row, 2].Text.Trim();
                if (kTps.Length == 1)
                {
                    kTps = "00" + kTps;
                }
                ...
                p1.tps = kTps;
                p1.nama = worksheet.Cells[row, 3].Text.Trim().ToUpper();
                kJenisKelamin = worksheet.Cells[row, 4].Text.Trim();
                if(kJenisKelamin.Length > 1)
                {
                    kJenisKelamin = kJenisKelamin.Substring(0, 1);
                }
                p1.jenis_kelamin = kJenisKelamin;
                p1.kelurahan = worksheet.Cells[row, 5].Text.Trim().ToUpper();
                p1.rt = worksheet.Cells[row, 6].Text.Trim();
                p1.rw = worksheet.Cells[row, 7].Text.Trim();
                p1.nik = worksheet.Cells[row, 8].Text.Trim().ToUpper().Replace("'", "").Replace("\"", "");
                p1.is_dukung = true;
                p1.namafolder = namafolder;
                p1.namafile = namafile;
                p1.nama_sheet = sheetName;
