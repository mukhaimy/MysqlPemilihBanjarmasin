...
{
    public class DptTpsOperationV2
    {
        ...
        public void TR2(string namaKota)
        {
            ...
            string baseFolderPath = @"D:\Development2023\15\DptOperation\by name DPT\Excel\1. tala\DPT EXCEL\";

            foreach (var folderItem in folderList)
            // string folderItem = folderList[8];
            {
                DirectoryInfo d = new DirectoryInfo(baseFolderPath + folderItem); //Assuming Test is your Folder
        ...
        public void T1(string namaKota, string filename)
        {
            ...
            {
                int nWorksheet = package.Workbook.Worksheets.Count;
                for (int i = 0; i < nWorksheet; i++)
                // for (int i = 0; i < 2; i++)
                {
                    
                    Console.WriteLine("~~~~~~~~~ SHEET: " + i.ToString());
                    var worksheetCurr = package.Workbook.Worksheets[i];
                    List<PemilihTps> lstPemilihTps = ReadDptWorksheet(worksheetCurr, namaKota);
        ...
        private List<PemilihTps> ReadDptWorksheet(ExcelWorksheet worksheet, string namaKota)
        {
            DateTime time0 = DateTime.Now;
        ...
        public List<string> GetNikList(MySqlConnection conn,
            string pnama, string pkelurahan, string pkecamatan, int pumur, string prt)
        {
            conn.Open();
            MySqlCommand cmd = new MySqlCommand("sp_get_nik", conn);
